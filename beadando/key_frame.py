...
class KeyFrame(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.key_frame_label = customtkinter.CTkLabel(master=self, text="API key", compound="left",
                                                      font=customtkinter.CTkFont(size=20))
        self.key_frame_label.grid(row=0, column=0, padx=20, pady=(200, 10))

        # Enter API key
        self.key_frame_entry = customtkinter.CTkEntry(master=self, placeholder_text="Your API key goes here", width=600,
                                                      font=customtkinter.CTkFont(size=16))
        self.key_frame_entry.grid(row=1, column=0, padx=10, pady=0)

        # Save API key button
        self.save_api_key_button = customtkinter.CTkButton(master=self, text="Save API key", width=150, height=40,
                                                           command=self.save_api_key_event, corner_radius=20,
                                                           font=customtkinter.CTkFont(size=16))
    ...
    def save_api_key(self, key_input):
        ...
        if check_api_key(key_input):
            ...
            self.key_frame_entry_success = customtkinter.CTkLabel(master=self, text="API key successfully set!",
                                                                  width=150, height=40, fg_color='#2c7d0a',
                                                                  corner_radius=20, font=customtkinter.CTkFont(size=16))
            ...
            self.key_frame_entry.configure(state='disabled', fg_color='#2c7d0a', text_color='#b8b2b2')
        else:
            self.key_frame_entry_fail = customtkinter.CTkLabel(master=self, text="Invalid API key!", width=150,
                                                               fg_color='#f51616', height=40, corner_radius=20,
                                                               font=customtkinter.CTkFont(size=16))
