...
class HomeFrame(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.new_idea_button = customtkinter.CTkButton(self, text="Create a new story!", command=self.new_idea,
                                                       width=150, height=50, font=customtkinter.CTkFont(size=16))
        self.new_idea_button.place(relx=0.5, rely=0.3, anchor='center')

    def new_idea(self):
        # Check if the user has inputted the API key
        if (get_api_key() is None):
            self.no_api_key_found = customtkinter.CTkLabel(master=self,
                                                           text="No API key found!\n\nPlease enter API key on the 'API key' tab!",
                                                           width=300, height=100, fg_color='#b211b8', corner_radius=15)
        ...
        self.new_idea_label = customtkinter.CTkLabel(master=self.new_idea_frame, text="Please enter your idea here!",
                                                     font=customtkinter.CTkFont(size=20, weight="bold"))
        self.new_idea_label.grid(row=1, column=0, padx=20, pady=10)

        # Idea entry box
        self.new_idea_entry_box = customtkinter.CTkTextbox(master=self.new_idea_frame, width=900, height=150,
                                                           font=customtkinter.CTkFont(size=15))
        self.new_idea_entry_box.grid(row=2, column=0, padx=20, pady=10)

        # Generate idea button
        self.generate_button = customtkinter.CTkButton(master=self.new_idea_frame, text="Generate",
    ...
    def generate(self):
        ...
        # Your prompt was...
        self.your_prompt = customtkinter.CTkLabel(master=self.generated_idea_frame, text="Your prompt was:",
                                                  font=customtkinter.CTkFont(size=20, weight="bold"))
        self.your_prompt.grid(row=1, column=0, padx=10, pady=10, sticky='ew')

        self.prompt = customtkinter.CTkTextbox(master=self.generated_idea_frame, width=800, height=80, wrap='word',
    ...
    def handle_response(self, response):
        ...
        self.response_label = customtkinter.CTkLabel(master=self.generated_idea_frame, text=f"{title}",
                                                     font=customtkinter.CTkFont(size=20, weight="bold"))
        self.response_label.grid(row=2, column=0, padx=10, pady=10, columnspan=2)

        self.response_text = customtkinter.CTkTextbox(master=self.generated_idea_frame, width=800, height=400,
                                                      wrap='word', font=customtkinter.CTkFont(size=18))
        ...
        self.generate_image_switch = customtkinter.CTkSwitch(master=self.generated_idea_frame,
        ...
        self.generate_image_label = customtkinter.CTkLabel(master=self.generated_idea_frame,
                                                           text="Enter the prompt for the image generation\n(feel free to change the title if you want to): ",
                                                           font=customtkinter.CTkFont(size=14))

        self.generate_image_entry = customtkinter.CTkEntry(master=self.generated_idea_frame, width=200,
                                                           font=customtkinter.CTkFont(size=16))
        self.generate_image_entry.insert(0, title)
        image_entry = self.generate_image_entry.get().strip()

        self.generate_image_entry_specify_further = customtkinter.CTkEntry(master=self.generated_idea_frame,
        ...
        # Show image in frame
        self.generate_image_button = customtkinter.CTkButton(self.generated_idea_frame, text="Generate image",
                                                             command=lambda: self.show_image(image_entry,
                                                                                             specification))

        self.save_response_button = customtkinter.CTkButton(self.generated_idea_frame, text="Save this Story",
                                                            command=lambda: self.save_response(title, response,
                                                                                               self.image_path))
        self.save_response_button.grid(row=9, column=0, padx=10, pady=10, sticky='')

        self.new_idea_button_on_error = customtkinter.CTkButton(self.generated_idea_frame, text="Try something else...",
                                                                command=self.new_idea)
        self.new_idea_button_on_error.grid(row=9, column=1, padx=10, pady=10, sticky='w')
    ...
    def error_in_response(self, error_message):
        self.error_in_response_label = customtkinter.CTkLabel(master=self.generated_idea_frame, text=f"{error_message}",
                                                              font=customtkinter.CTkFont(size=14), width=200,
                                                              fg_color='#f51616', corner_radius=20)
        self.error_in_response_label.grid(row=2, columnspan=2)

        self.new_idea_button_on_error = customtkinter.CTkButton(self.generated_idea_frame, text="Try again",
                                                                command=self.new_idea)
    ...
    def save_response(self, title, response, image_path):
        ...
        if os.path.exists(file_path):
            ...
            for story in stories:
                if (story_data.get("title") == story.get("title") and story_data.get("content")[0:10] == story.get(
                        "content")[0:10]):
                    already_exists = True
            ...
            if not already_exists:
                ...
                self.story_save_success = customtkinter.CTkLabel(master=self.generated_idea_frame,
                                                                 text="Your story has been saved!", width=120,
                                                                 height=30, fg_color='#2c7d0a', corner_radius=15,
                                                                 font=customtkinter.CTkFont(size=16))
                self.story_save_success.grid(row=10, column=0, padx=10, pady=10, columnspan=2)
            else:
                self.story_save_error = customtkinter.CTkLabel(master=self.generated_idea_frame,
                                                               text="This story has already been saved!", width=120,
                                                               height=30, fg_color='#b8b2b2', corner_radius=15,
                                                               font=customtkinter.CTkFont(size=16))
                self.story_save_error.grid(row=10, column=0, padx=10, pady=10, columnspan=2)
        else:
    ...
    def switch_event(self):
        ...
        if (self.switch_var.get() == "on"):
            self.generate_image_label.grid(row=5, column=0, padx=10, pady=10, columnspan=2)
            self.generate_image_entry.grid(row=6, column=0, padx=10, pady=10, columnspan=2)
            self.generate_image_entry_specify_further.grid(row=7, column=0, columnspan=2, padx=10, pady=10)
            self.generate_image_button.grid(row=8, column=0, columnspan=2, padx=10, pady=10)
