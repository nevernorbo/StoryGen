...
class PreviousGenerationsFrame(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.no_stories_yet = customtkinter.CTkLabel(
            self,
            text="You don't have any saved stories yet!\n",
            width=150,
            height=50,
    ...
    def get_stories(self):
        ...
        for i, story in enumerate(data):
            ...
            title_label = customtkinter.CTkLabel(
                master=frame, text=f"Title: {story['title']}"
            )
            date_label = customtkinter.CTkLabel(
                master=frame, text=f"Date created: {story['date created']}"
            )

            # textbox for story content
            story_textbox = customtkinter.CTkTextbox(
                master=frame, wrap="word", height=300
            )
            ...
            if story["image_path"] != "":
                view_image_button = customtkinter.CTkButton(
                    master=frame,
                    text="View image",
                    command=lambda image_path=story["image_path"]: self.open_image(
                        image_path
                    ),
                )
                view_image_button.grid(
                    row=i * 3 + 3, column=0, sticky="w", padx=10, pady=10
                )

            title_label.grid(row=i * 3 + 1, column=0, sticky="w", padx=10, pady=10)
            date_label.grid(row=i * 3 + 2, column=0, sticky="w", padx=10, pady=10)
            story_textbox.grid(
                row=i * 3 + 4, column=0, sticky="nsew", padx=10, pady=(10, 20)
            )
